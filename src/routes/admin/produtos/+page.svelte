<script lang="ts">
	import produtoImg from '$lib/img/produtosadmin.jpg';
...
<div class="grid grid-cols-1 gap-7 py-24 md:grid-cols-2 lg:grid-cols-3">
	{#if form?.body.message != "" && form?.body.message != undefined}
		<!-- <Modal tipo={TipoModal.OK} showModal={form?.message != ""}> -->
		<Modal showModal={form?.body.message != ""}>
			<h2 slot="header">
				{form?.body.message}
			</h2>
		</Modal>
	...
	{#each produtos as produto}
		<form method="post" action="?/editarEstoque">
			...
			<div class="mx-auto flex w-full max-w-xs flex-col items-center justify-center">
				<div
					...
					style="background-image: url({produtoImg})">
				</div>
				...
				<div class="-mt-10 w-56 overflow-hidden rounded-lg bg-white shadow-lg md:w-64">
					...
					<div class="flex items-center justify-between bg-seashell px-3 py-2">
						...
						<CounterInput count="{produto.estoque}" inputName="estoque"/>
						<button
							type="submit"
					...
					<div class="flex items-center justify-between bg-seashell px-3 py-2">
						<span class="font-semibold text-gray-800">{formatarValor(produto.valor)}</span>
