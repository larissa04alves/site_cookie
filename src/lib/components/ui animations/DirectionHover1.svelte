<script lang="ts">
	...
	import cookieNutella from '$lib/img/cookieNutella.png';
...
<div class="flex h-[60vh] items-center justify-center">
	<Motion let:motion>
		...
		<div
			use:motion
			class={cn(
				'group/card relative h-60 w-60 overflow-hidden rounded-lg bg-transparent md:h-[45vh] md:w-[55vh]',
				className
			)}
			bind:this={ref}
			...
			<AnimatePresence let:item list={[{ key: 's' }]}>
				<Motion initial="initial" whileHover={$direction} exit="exit" let:motion>
					<div class="relative h-full w-full" use:motion>
						<!-- Tag de Top 1 -->
						<div
							class="absolute left-2 top-2 z-20 rounded-full bg-green-700 px-3 py-1 text-sm font-semibold text-white"
						>
							Top 1
						</div>
						<div
